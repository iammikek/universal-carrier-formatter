...
class Parameter(BaseModel):
    ...
    enum_values: Optional[List[Any]] = Field(None, alias="enum", description="Allowed enum values")
    ...
    class Config:
        ...
        allow_population_by_field_name = True  # Allow both 'default' and 'default_value'
        json_schema_extra = {
            "example": {
...
class RequestSchema(BaseModel):
    ...
    body_schema: Optional[Dict[str, Any]] = Field(None, description="JSON schema for request body")
...
class ResponseSchema(BaseModel):
    ...
    body_schema: Optional[Dict[str, Any]] = Field(None, description="JSON schema for response body")
...
class Endpoint(BaseModel):
    ...
    """
    path: str = Field(..., description="API endpoint path (e.g., /api/v1/track)", min_length=1)
    ...
    responses: List[ResponseSchema] = Field(default_factory=list, description="Possible responses")
    authentication_required: bool = Field(default=False, description="Does endpoint require auth")
    tags: List[str] = Field(default_factory=list, description="Endpoint tags/categories")
...
class AuthenticationMethod(BaseModel):
    ...
    parameter_name: Optional[str] = Field(None, description="Parameter name (e.g., 'X-API-Key')")
...
class RateLimit(BaseModel):
    ...
    period: str = Field(..., description="Time period (e.g., '1 minute', '1 hour', '1 day')")
...
class UniversalCarrierFormat(BaseModel):
    ...
    # Metadata
    documentation_url: Optional[HttpUrl] = Field(None, description="Link to original documentation")
